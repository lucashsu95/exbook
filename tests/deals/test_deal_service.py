...
class TestCreateDeal:
    ...
    def test_loan_success(self):
        ...
        book = SharedBookFactory(
            owner=owner, status='T', transferability='RETURN',
    ...
    def test_transfer_success(self):
        ...
        applicant = UserFactory()
        book = SharedBookFactory(
            owner=owner, status='T', transferability='TRANSFER',
        )
    ...
    def test_restore_success(self):
        ...
        applicant = UserFactory()
        book = SharedBookFactory(
            owner=owner, status='R', transferability='RETURN',
        )
    ...
    def test_regress_success(self):
        ...
        owner = UserFactory()
        book = SharedBookFactory(
            owner=owner, keeper=keeper, status='T', transferability='TRANSFER',
        )
    ...
    def test_br7_book_set_incomplete_raises(self):
        ...
        SharedBookFactory(
            owner=owner, status='S', transferability='RETURN',
            book_set=book_set,
        )
...
class TestCancelDeal:
    ...
    def test_br14_restore_book_status(self):
        ...
        book = SharedBookFactory(status='T', transferability='RETURN')
        deal = DealFactory(
            shared_book=book, status='Q', previous_book_status='T',
        )
        book.status = 'V'
...
class TestCompleteMeeting:
    ...
    def test_loan_meeting(self):
        ...
        applicant = UserFactory()
        book = SharedBookFactory(
            owner=owner, keeper=owner, status='V',
            transferability='RETURN', loan_duration_days=30,
        )
        deal = DealFactory(
            shared_book=book, deal_type='LN', status='P',
    ...
    def test_transfer_meeting(self):
        ...
        book = SharedBookFactory(
            owner=owner, keeper=owner, status='V',
            transferability='TRANSFER', loan_duration_days=30,
        )
        deal = DealFactory(
            shared_book=book, deal_type='TF', status='P',
            applicant=applicant, responder=owner,
        )
        complete_meeting(deal)
        book.refresh_from_db()
    ...
    def test_restore_meeting(self):
        ...
        applicant = UserFactory()
        book = SharedBookFactory(
            owner=owner, keeper=applicant, status='V',
            transferability='RETURN',
        )
        deal = DealFactory(
            shared_book=book, deal_type='RS', status='P',
            applicant=applicant, responder=owner,
        )
        complete_meeting(deal)
        book.refresh_from_db()
...
class TestProcessBookDue:
    ...
    def test_return_book_overdue(self):
        ...
        deal = DealFactory(
            shared_book=book, deal_type='LN', status='M',
    ...
    def test_transfer_book_overdue(self):
        ...
        deal = DealFactory(
            shared_book=book, deal_type='TF', status='M',
            due_date=timezone.now().date() - timedelta(days=1),
        )
        process_book_due(deal)
        book.refresh_from_db()
    ...
    def test_non_meeted_no_change(self):
        book = SharedBookFactory(status='O', transferability='RETURN')
        deal = DealFactory(
            shared_book=book, deal_type='LN', status='D',
            due_date=timezone.now().date() - timedelta(days=1),
        )
        process_book_due(deal)
        book.refresh_from_db()
    ...
    def test_notification_sent(self):
        ...
        keeper = UserFactory()
        book = SharedBookFactory(
            owner=owner, keeper=keeper, status='O', transferability='RETURN',
        )
        deal = DealFactory(
            shared_book=book, deal_type='LN', status='M',
            due_date=timezone.now().date() - timedelta(days=1),
